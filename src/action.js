...
function makeGroup(k, children) {
    return k === "and" ? { and: children } : { or: children };
}
/**
 ...
 */
function updateGroupAt(root, path, updater) {
    ...
    for (let depth = 0; depth < path.length; depth++) {
        ...
        if (step.index < 0 || step.index >= list.length) {
            throw new Error(`Index out of range at depth ${depth}: ${k}[${step.index}], length=${list.length}`);
        }
    ...
    for (let i = parents.length - 1; i >= 0; i--) {
        ...
        const list = childrenOf(group).slice();
        list[index] = updated;
...
export function addConditionAt(root, groupPath = [], condition) {
...
export function addGroupAt(root, groupPath = [], newKey) {
...
export function removeAt(root, nodePath) {
...
export function toggleGroupKeyAt(root, groupPath = []) {
    return updateGroupAt(root, groupPath, (g) => {
        const list = childrenOf(g);
        const nextKey = keyOf(g) === "and" ? "or" : "and";
        return makeGroup(nextKey, list);
    });
}
// Update an existing CONDITION at `nodePath` with a partial patch
export function updateConditionAt(root, nodePath, patch) {
    if (!Array.isArray(nodePath) || nodePath.length === 0) {
...
export function replaceNodeAt(root, nodePath, node) {
    if (!Array.isArray(nodePath) || nodePath.length === 0) {
